...
def main() -> None:
    ...
    commands = {
        ...
        'cp': fs.command_cp,
        'mv': fs.command_mv,
        'history': plugins.command_history,
        'undo': plugins.command_undo,
        'zip': plugins.command_zip,
        'unzip': plugins.command_unzip,
        'tar': plugins.command_tar,
        'untar': plugins.command_untar,
    }
    ...
    while True:
        ...
        if name_command not in commands:
            ...
            store.log_error(f'ERROR: Неизвестная команда: {name_command}.')
            continue
