...
- Написать модульные тесты на PyTest для всех основных классов и функции симуляции.
...
│   ├── tests/
│   │   ├── test_models.py      # Тесты для моделей книг
│   │   ├── test_collections.py # Тесты для пользовательских коллекций и индексов
│   │   ├── test_library.py     # Тесты для класса Library
│   │   └── test_simulation.py  # Тесты для функции run_simulation и логирования
│   ├── .gitignore              # Игнорируемые файлы для git
│   ├── .pre-commit-config.yaml # Настройки линтеров и автоформатирования
│   ├── requirements.txt        # Зависимости проекта (в т.ч. pytest)
...
- функцию `run_simulation(steps: int = 50, seed: int | None = None)`, которая:
  ...
  - случайным образом выбирает операции (ADD, REMOVE, FIND\_*);
...
Автотесты лежат в каталоге `tests/` и написаны с использованием PyTest:

- `test_models.py` — проверка инициализации и валидации классов `Book`, `FictionBook`, `ScienceBook`.  
- `test_collections.py` — тестируются операции с `BookCollection` и `IndexDict` (добавление, удаление, поиск, ошибки).  
- `test_library.py` — сценарии работы класса `Library` (инициализация, добавление/удаление, поиск по разным полям).  
- `test_simulation.py` — проверка параметров `run_simulation`, работы с seed и факта записи лога.

Запуск тестов из корня лабораторной:

```bash
pytest -v
```

Тесты должны успешно проходить при корректной установке зависимостей.
